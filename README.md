...
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:7c3aed,100:14b8a6&height=200&section=header&text=dscache&fontSize=72&fontColor=ffffff&desc=DeepSeek%20prefix-cache%20%E5%88%A9%E6%B6%A6%E8%A1%A8&descSize=20&descAlignY=72" alt="dscache" width="100%" />
</p>
...
```
你的 Agent 代码
      │  client = dscache.wrap(OpenAI(...))      # 纯透传，绝不改写请求
      ▼
┌─────────────┐   读取 usage.prompt_cache_hit_tokens / miss_tokens
│  wrapper.py │ ──────────────────────────────────────────────┐
└─────────────┘                                                ▼
                                              .dscache/ledger.jsonl  (本地账本)
                                                               │
                                                               ▼
                                                       ┌─────────────┐
                                                       │ profiler.py │  逐请求定档 HIT/PARTIAL/MISS
                                                       │             │  + prefix 指纹 + 两档计价
                                                       └──────┬──────┘
                                          ┌───────────────────┴──────────────────┐
                                          ▼                                       ▼
...
单一 Python 包，单进程，无守护进程、无服务端、无我们自己的网络调用。数据流向：你的代码 → `wrapper` → `ledger.jsonl` → `profiler` →（`report` | `reorder`）→ 终端。
...
完整的 30 秒演示脚本见 [`assets/demo.cast`](./assets/demo.cast)（asciinema 录像）与 [`docs/demo.tape`](./docs/demo.tape)（vhs 脚本，CI 会在打 tag 时渲染成 `assets/demo.gif`）。
...
- [ ] **m2 · 重排建议** — `profiler` 计算前缀指纹、标记打爆事件；`dscache suggest` 给出能恢复稳定缓存段的具体重排。
- [ ] **m3 · 金钱头条** — 汇总 `cost_actual − cost_ideal`，产出一句可分享的「打爆 N×，多花 X.Y 倍，浪费 ¥Z」。
- [ ] 托管团队版（上传聚合 + 回退告警，见上）。
